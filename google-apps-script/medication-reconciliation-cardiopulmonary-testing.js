// Google Apps Script - Medication Reconciliation (Combined Cardiopulmonary Testing)
...
function doPost(e) {
  try {
    ...
    // Document name
    const docName = `Medication_Reconciliation_Cardiopulmonary_Testing_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`;
    ...
    if (Array.isArray(formData.currentMedications) && formData.currentMedications.length > 0) {
      ...
      medsTable.setBorderWidth(1).setBorderColor("#000000");
    ...
    if (Array.isArray(formData.pastMedicalHistory) && formData.pastMedicalHistory.length > 0) {
      ...
      histTable.setBorderWidth(1).setBorderColor("#000000");
    ...
    return ContentService.createTextOutput(JSON.stringify({
      ...
      driveUrl: driveUrl,
      docId: doc.getId(),
      message: "Medication Reconciliation (Combined Cardiopulmonary Testing) submitted and saved to Google Drive successfully"
    })).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      error: error.toString(),
      message: "Failed to process Medication Reconciliation (Combined Cardiopulmonary Testing)"
    })).setMimeType(ContentService.MimeType.JSON);
  }
}
...
function doGet() {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: "Medication Reconciliation (Combined Cardiopulmonary Testing) Script is running",
