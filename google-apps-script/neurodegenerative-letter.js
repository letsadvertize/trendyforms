// Google Apps Script Code - Letter of Medical Necessity (Neurodegenerative)
// Deploy this at scripts.google.com

// Neurodegenerative folder ID from your Google Drive
const NEURODEGENERATIVE_FOLDER_ID = "1rEAHZCTCL1dHSaT3Z68T2gxRJSBP0836" // Update this with your folder ID
...
function doPost(e) {
  try {
    ...
    const docName = `Letter_Medical_Necessity_Neurodegenerative_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`
    ...
    body.appendParagraph(`I am writing on behalf of my patient, ${formData.patientName || "Patient Name"}, to document the medical necessity of Neurodegenerative genetic testing.`)
      .setFontSize(11)
      .setSpacingAfter(8)

    body.appendParagraph(`I have determined that this test is medically necessary for the above patient due to the following history, which is strongly indicative of genetic etiology or a hereditary neurodegenerative disease consistent with a mutation in multiple genes.`)
      .setFontSize(11)
      .setSpacingAfter(12)
    ...
    if (formData.patientConditions && Array.isArray(formData.patientConditions) && formData.patientConditions.length > 0) {
      ...
      headerCell1.clear()
      headerCell1.appendParagraph("Neurodegenerative Conditions")
        .setFontSize(11)
        .setBold(true)
    ...
    const medicalNecessityText = `If a mutation is found in one or more genes on the Neurodegenerative genetic test, it would provide a diagnosis of a hereditary Neurodegenerative condition and thereby helping to clarify the patient's risk and prompting a change in the patient's medical management. Accurate diagnosis will undoubtedly save the patient undue suffering and treatment delays. This information allows healthcare providers to better inform affected individuals and families about prognosis and optimal surveillance strategies and may also guide therapy. Understanding the etiology of the symptoms will enable avoidance of inpatient hospitalization. Patients with neurodegenerative conditions might have similar phenotypes even though they have variants in different genes. Knowing which gene is affected helps determine whether the best treatment option might be targeted therapy, neuroprotective agents, or symptomatic management. In addition, once the molecular diagnosis is established, healthcare providers can better define the risk of recurrence and decide whether to screen unaffected family members for carrier status, to provide genetic counseling, or for other purposes.`
    ...
    // CONCLUSION SECTION
    body.appendParagraph("Based on my evaluation and review of the available literature, I believe that the Neurodegenerative genetic testing offered by Hospital In Your Home HIYH is warranted and medically necessary for my patient.")
      .setFontSize(11)
      .setBold(false)
      .setSpacingBefore(12)
      .setSpacingAfter(16)
    ...
    const fileName = `Letter_Medical_Necessity_Neurodegenerative_${patientName}_${date}_${time}.pdf`
    ...
    const response = {
      ...
      docId: doc.getId(),
      message: "Letter of Medical Necessity (Neurodegenerative) submitted and saved to Google Drive successfully"
    }
    ...
    console.error("Error processing neurodegenerative letter:", error)
    
    const errorResponse = {
      success: false,
      error: error.toString(),
      message: "Failed to process Letter of Medical Necessity (Neurodegenerative)",
      stack: error.stack
    }
...
function doGet() {
  ...
  return ContentService.createTextOutput(JSON.stringify({
    message: "Neurodegenerative Letter Google Apps Script is running",
