// Google Apps Script - Medication Reconciliation (Diabetes MODY & Predict)
...
function doPost(e) {
  try {
    ...
    // Document name
    const docName = `Medication_Reconciliation_Diabetes_MODY_Predict_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`;
    ...
    if (DIABETES_MODY_PREDICT_FOLDER_ID) {
      try {
    ...
    return ContentService.createTextOutput(JSON.stringify({
      ...
      driveUrl: driveUrl,
      docId: doc.getId(),
      message: "Medication Reconciliation (Diabetes MODY & Predict) submitted and saved to Google Drive successfully"
    })).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      error: error.toString(),
      message: "Failed to process Medication Reconciliation (Diabetes MODY & Predict)"
    })).setMimeType(ContentService.MimeType.JSON);
  }
}
...
function doGet() {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: "Medication Reconciliation (Diabetes MODY & Predict) Script is running",
