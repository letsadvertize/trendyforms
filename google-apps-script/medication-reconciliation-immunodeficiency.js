// Google Apps Script Code - Medication Reconciliation (Immunodeficiency)
...
function doPost(e) {
  try {
    ...
    const docName = `Medication_Reconciliation_Immunodeficiency_${fields.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`
    ...
    const subtitlePara = titleCell.appendParagraph("Immunodeficiency Specialty")
    ...
    body.appendParagraph(`Form ID: MRI-${Date.now()}`)
      .setAlignment(DocumentApp.HorizontalAlignment.CENTER)
      .setFontSize(9)
    ...
    const pdfName = `Medication_Reconciliation_Immunodeficiency_${patientName}_${date}_${time}.pdf`
    ...
    console.log("Medication Reconciliation (Immunodeficiency) processed successfully")
    ...
    const response = {
      ...
      driveUrl: driveUrl,
      docId: doc.getId(),
      message: "Medication Reconciliation (Immunodeficiency) submitted and saved to Google Drive successfully"
    }
    ...
    console.error("Error processing Medication Reconciliation (Immunodeficiency):", error)
    
    const errorResponse = {
      success: false,
      error: error.toString(),
      message: "Failed to process Medication Reconciliation (Immunodeficiency)",
      stack: error.stack
    }
...
function doGet() {
  return ContentService.createTextOutput(JSON.stringify({
    message: "Immunodeficiency Medication Reconciliation Google Apps Script is running",
...
function testScript() {
    ...
    postData: {
      contents: JSON.stringify({
        ...
        formData: {
          ...
          patientAddress: "123 Main St, Anytown, AT 12345",
          currentMedications: [
            { name: "Metformin", dosage: "500mg", frequency: "Twice daily" },
            { name: "Lisinopril", dosage: "10mg", frequency: "Once daily" }
          ],
          pastMedicalHistory: [
            { condition: "Diabetes Type 2", diagnosisDate: "2020-01-01", status: "Controlled" },
            { condition: "Hypertension", diagnosisDate: "2019-05-15", status: "Controlled" }
          ]
        },
        timestamp: new Date().toISOString()
      })
  ...
  console.log("Testing Medication Reconciliation (Immunodeficiency) script...")
