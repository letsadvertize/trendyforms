// Google Apps Script Code - Provider Visit Attestation (Other Providers)
// Deploy this at scripts.google.com

// Set your Google Drive folder ID for Other Providers
...
function doPost(e) {
  try {
    ...
    const docName = `Provider_Visit_Attestation_Other_Providers_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`
    ...
    if (OTHER_PROVIDERS_FOLDER_ID && OTHER_PROVIDERS_FOLDER_ID !== "YOUR_OTHER_PROVIDERS_FOLDER_ID") {
      try {
    ...
    return ContentService.createTextOutput(JSON.stringify({
      ...
      driveUrl: driveUrl,
      docId: doc.getId(),
      message: "Provider Visit Attestation (Other Providers) submitted and saved to Google Drive successfully"
    })).setMimeType(ContentService.MimeType.JSON)
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      error: error.toString(),
      message: "Failed to process Provider Visit Attestation (Other Providers)"
    })).setMimeType(ContentService.MimeType.JSON)
  }
}
...
function doGet() {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: "Provider Visit Attestation (Other Providers) Script is running",
