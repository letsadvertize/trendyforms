// Google Apps Script - Progress Note (Hereditary Thyroid)
...
function doPost(e) {
  try {
    ...
    // Document name
    const docName = `Progress_Note_Hereditary_Thyroid_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`;
    ...
    visitTable.setBorderWidth(1).setBorderColor("#e2e8f0");
    body.appendParagraph("");
    ...
    const fileName = `Progress_Note_Hereditary_Thyroid_${patientName}_${date}_${time}.pdf`;
    ...
    const response = {
      ...
      docId: doc.getId(),
      message: "Progress Note (Hereditary Thyroid) submitted and saved to Google Drive successfully"
    };
    ...
    const errorResponse = {
      ...
      message: "Failed to process Progress Note (Hereditary Thyroid)",
      stack: error.stack
    };
...
function doGet() {
  return ContentService.createTextOutput(JSON.stringify({
    message: "Hereditary Thyroid Progress Note Google Apps Script is running",
