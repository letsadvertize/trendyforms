// Google Apps Script Code - Patient Progress Note (Neurodegenerative)
...
function doPost(e) {
  try {
    ...
    const docName = `Progress_Note_Neurodegenerative_${formData.patientName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}_${new Date().toLocaleTimeString().replace(/:/g, '-')}`;
    ...
    const titlePara = titleCell.appendParagraph("PROGRESS NOTE - NEURODEGENERATIVE PATIENT");
    ...
    const fileName = `Progress_Note_Neurodegenerative_${patientName}_${date}_${time}.pdf`;
    ...
    return ContentService
      .createTextOutput(JSON.stringify({
        success: true,
        message: 'Progress Note (Neurodegenerative) submitted successfully',
        fileName: fileName,
    ...
    return ContentService
      .createTextOutput(JSON.stringify({
        ...
        message: 'Failed to process Progress Note (Neurodegenerative)'
      }))
      .setMimeType(ContentService.MimeType.JSON);
  }
}
...
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: 'Patient Progress Note - Neurodegenerative Script is running',
