// Google Apps Script - Progress Note (Combined Cardiopulmonary Testing)
// Deploy as Web App (Anyone), set CARDIOPULMONARY_TESTING_FOLDER_ID below
...
function doPost(e) {
  try {
    ...
    // Document name
    const docName = `Progress_Note_Cardiopulmonary_Testing_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`;
    ...
    const fileName = `Progress_Note_Cardiopulmonary_Testing_${patientName}_${date}_${time}.pdf`;
    ...
    const response = {
      ...
      docId: doc.getId(),
      message: "Progress Note (Combined Cardiopulmonary Testing) submitted and saved to Google Drive successfully"
    };
    ...
    const errorResponse = {
      ...
      message: "Failed to process Progress Note (Combined Cardiopulmonary Testing)",
      stack: error.stack
    };
...
function doGet() {
  return ContentService.createTextOutput(JSON.stringify({
    message: "Combined Cardiopulmonary Testing Progress Note Google Apps Script is running",
