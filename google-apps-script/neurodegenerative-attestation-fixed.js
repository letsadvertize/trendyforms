// Google Apps Script Code - Provider Visit Attestation (Neurodegenerative) - FIXED VERSION
...
function doPost(e) {
  try {
    ...
    const docName = `Provider_Visit_Attestation_Neurodegenerative_${fields.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`
    ...
    // Build the attestation statement with actual data (adapted for Neurodegenerative)
    ...
    const attestationText = `I am the treating physician for ${patientNameText}. I have ordered a Comprehensive Neurodegenerative genetic test on ${consultDateText}. The original requisition form signed by myself and the patient is attached for review.
...
I do hereby attest that this information is true, accurate and complete to the best of my knowledge.`
    ...
    body.appendParagraph(`Form ID: PVAN-${Date.now()}`)
      .setAlignment(DocumentApp.HorizontalAlignment.CENTER)
      .setFontSize(9)
    ...
    const pdfName = `Provider_Visit_Attestation_Neurodegenerative_${patientName}_${date}_${time}.pdf`
    ...
    const response = {
      ...
      driveUrl: driveUrl,
      docId: doc.getId(),
      message: "Provider Visit Attestation (Neurodegenerative) submitted and saved to Google Drive successfully"
    }
    ...
    console.error("Error processing Provider Visit Attestation (Neurodegenerative):", error)
    
    const errorResponse = {
      success: false,
      error: error.toString(),
      message: "Failed to process Provider Visit Attestation (Neurodegenerative)",
      stack: error.stack
    }
...
function doGet() {
  return ContentService.createTextOutput(JSON.stringify({
    message: "Neurodegenerative Attestation Google Apps Script is running",
...
function testScript() {
    ...
    postData: {
      contents: JSON.stringify({
        ...
        formData: {
          ...
          providerNameSignature: "Dr. Jane Smith, MD",
  ...
  console.log("Testing Provider Visit Attestation (Neurodegenerative) script...")
