// Google Apps Script Code - Letter of Medical Necessity (Diabetes MODY & Predict)
// Deploy this at scripts.google.com

// Diabetes MODY & Predict folder ID from your Google Drive
const DIABETES_MODY_PREDICT_FOLDER_ID = "YOUR_FOLDER_ID_HERE"; // Update this with your folder ID
...
function doPost(e) {
  try {
    ...
    const docName = `Letter_Medical_Necessity_Diabetes_MODY_Predict_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`;
    const doc = DocumentApp.create(docName);
    const body = doc.getBody();

    // HEADER SECTION (Simple Title Only)
    body.appendParagraph("Letter of Medical Necessity: Diabetes MODY & Predict")
    ...
    body.appendParagraph(`I am writing on behalf of my patient, ${formData.patientName || "Patient Name"}, to document the medical necessity of MODY & Predict diabetes genetic testing.`).setFontSize(11).setSpacingAfter(8);
    body.appendParagraph(`I have determined that this test is medically necessary for the above patient due to the following history, which is strongly indicative of a genetic etiology or a hereditary MODY type: ${formData.modyType || "MODY"} consistent with a mutation in multiple genes.`).setFontSize(11).setSpacingAfter(12);
  ...
  body.appendParagraph("Patient Personal History").setHeading(DocumentApp.ParagraphHeading.HEADING1).setFontSize(14).setBold(true).setForegroundColor("#000000").setSpacingBefore(16).setSpacingAfter(8);
    if (formData.patientConditions && Array.isArray(formData.patientConditions) && formData.patientConditions.length > 0) {
      ...
      headerCell1.clear();
  headerCell1.appendParagraph("Diabetes Conditions").setFontSize(11).setBold(true).setForegroundColor("#000000");
  ...
  body.appendParagraph("Additional Information & Medical Necessity").setHeading(DocumentApp.ParagraphHeading.HEADING1).setFontSize(14).setBold(true).setForegroundColor("#000000").setSpacingBefore(16).setSpacingAfter(8);
    ...
    body.appendParagraph("Based on my evaluation and review of the available literature, I believe that MODY & Predict diabetes genetic testing is warranted and medically necessary for my patient.").setFontSize(11).setBold(false).setSpacingBefore(12).setSpacingAfter(16);
  ...
  body.appendParagraph("Provider Information").setHeading(DocumentApp.ParagraphHeading.HEADING1).setFontSize(14).setBold(true).setForegroundColor("#000000").setSpacingBefore(16).setSpacingAfter(8);
    ...
    const fileName = `Letter_Medical_Necessity_Diabetes_MODY_Predict_${patientName}_${date}_${time}.pdf`;
    ...
    const response = {
      ...
      docId: doc.getId(),
      message: "Letter of Medical Necessity (Diabetes MODY & Predict) submitted and saved to Google Drive successfully"
    };
    ...
    const errorResponse = {
      ...
      message: "Failed to process Letter of Medical Necessity (Diabetes MODY & Predict)",
      stack: error.stack
    };
...
function doGet() {
  return ContentService.createTextOutput(JSON.stringify({
    message: "Diabetes MODY & Predict Letter Google Apps Script is running",
