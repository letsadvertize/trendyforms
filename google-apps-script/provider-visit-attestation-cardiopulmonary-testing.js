// Google Apps Script Code - Provider Visit Attestation (Combined Cardiopulmonary Testing)
// Deploy this at scripts.google.com

// Set your Google Drive folder ID for Combined Cardiopulmonary Testing
const CARDIOPULMONARY_TESTING_FOLDER_ID = "1k9cXTb0pUerXvsdHxRWGN2kSybjRm5j-"; // <-- Set your folder ID here
...
function doPost(e) {
  try {
    ...
    const docName = `Provider_Visit_Attestation_Cardiopulmonary_Testing_${formData.patientName || 'Unknown'}_${new Date().toISOString().split('T')[0]}`;
    ...
    body.appendParagraph("Provider Visit Attestation")
      ...
      .setBold(true)
      .setFontSize(18);
    body.appendParagraph("Combined Cardiopulmonary Testing Specialty")
      .setFontSize(12)
      .setForegroundColor("#000000");
    ...
    referringTable.appendTableRow()
      ...
      .setFontSize(11);
    referringTable.setBorderWidth(1).setBorderColor("#000000");
    body.appendParagraph("");
    ...
    testTable.appendTableRow()
      ...
      .setFontSize(11);
    testTable.setBorderWidth(1).setBorderColor("#000000");
    body.appendParagraph("");
    ...
    const attestationText = [
      `I am the treating provider for ${formData.patientName || "[PATIENT NAME]"}. I have ordered a cardiopulmonary test on ${formData.dateOfConsult || "[DATE OF CONSULT]"}. The original requisition form signed by myself and the patient is attached for review.`,
      `The test was ordered based on the following diagnosis as identified on the requisition (${formData.icdCodes || "[ICD 10-CODES]"})`,
      "The specimen was provided as of the actual date of service by the patient and sent directly to the lab.",
    ...
    sigTable.appendTableRow()
      ...
      .setFontSize(11);
    sigTable.setBorderWidth(1).setBorderColor("#000000");
    body.appendParagraph("");
    ...
    body.appendParagraph("© " + new Date().getFullYear() + " Combined Cardiopulmonary Testing. All rights reserved.")
      .setFontSize(8)
      .setForegroundColor("#000000")
      .setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    ...
    if (CARDIOPULMONARY_TESTING_FOLDER_ID) {
      try {
    ...
    return ContentService.createTextOutput(JSON.stringify({
      ...
      driveUrl: driveUrl,
      docId: doc.getId(),
      message: "Provider Visit Attestation (Combined Cardiopulmonary Testing) submitted and saved to Google Drive successfully"
    })).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
      success: false,
      error: error.toString(),
      message: "Failed to process Provider Visit Attestation (Combined Cardiopulmonary Testing)"
    })).setMimeType(ContentService.MimeType.JSON);
  }
}
...
function doGet() {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: "Provider Visit Attestation (Combined Cardiopulmonary Testing) Script is running",
